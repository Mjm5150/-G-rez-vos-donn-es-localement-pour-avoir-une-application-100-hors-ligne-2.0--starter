...
public class ItemContentProvider extends ContentProvider {
    ...
    public Cursor query(@NonNull Uri uri, @Nullable String[] projection, @Nullable String selection,
                        @Nullable String[] selectionArgs, @Nullable String sortOder) {

        if (getContext() != null) {

            long userId = ContentUris.parseId(uri);
            final Cursor cursor = SaveMyTripDatabase.getInstance(getContext()).itemDao()
    ...
    public int update(@NonNull Uri uri, @Nullable ContentValues contentValues, @Nullable String s,
        ...
        if (getContext() != null && contentValues != null) {
            final int count = SaveMyTripDatabase.getInstance(getContext()).itemDao()
                    .updateItem(Item.fromContentValues(contentValues));
            getContext().getContentResolver().notifyChange(uri, null);
            return count;
        }
